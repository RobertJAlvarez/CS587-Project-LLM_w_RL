...
class TextDataset(Dataset):
    def __init__(self, file_path, tokenizer, block_size=128) -> None:
        ...
        self.tokens = self.tokenizer.encode(
            processed_data, allowed_special={"<|endoftext|>"}
        )
...
class RotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048) -> None:
        super().__init__()
        self.dim = dim
    ...
    def forward(self, seq_len):
        ...
        # Compute sin and cos for positional encoding. Shape: (seq_len, dim/2).
...
def apply_rotary_emb(q, k, sin, cos):
    ...
    # NOTE: Apply rotation using the rotation matrix?

    # Rotate query states (q) and key states (k) by sin and cos.
    q_rotated = torch.zeros_like(q)
    if q.size(2) == 1:
        # If q's seq_len == 1, this means we are using KV cache. Apply the last position's sin and cos to q.
    ...
    # Rotate key states (k). Note that k will always has the full sequence length
    #   (i.e. during generation, KV cache + 1 new token), so no need for special handling.
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None, use_cache=False):
        ...
        q = q.view(B, T, self.n_head, self.split_size).transpose(1, 2)
        k = k.view(B, T, self.n_head, self.split_size).transpose(1, 2)
        v = v.view(B, T, self.n_head, self.split_size).transpose(1, 2)
        ...
        att = torch.matmul(q, k.transpose(-2, -1)) / (self.split_size**0.5)

        # Apply causal mask - using the appropriate size for this sequence.
        # When using KV cache during generation, no need to mask because the new
        #   token's query should be allowed to attend to all past and current keys.
        if layer_past is None:
            att = att.masked_fill(self.mask[:, :, :T, :T] == 0, float("-inf"))

        # Apply softmax to get attention weights.
        att = self.attn_dropout(torch.nn.functional.softmax(att, dim=-1))
    ...
    def forward(self, idx, past_key_values=None, use_cache=False):
        ...
        if past_key_values is None:
            ...
            assert len(past_key_values) == len(
                self.transformer.h
            ), "Past key values length must match number of layers"
            if past_key_values[0] is not None:
                # Get the sequence length from the first layer's KV cache.
                past_length = past_key_values[0][0].size(2)
                assert (
                    past_length + t <= self.config.block_size
                ), f"Cannot forward sequence of total length {past_length + t}, block size is only {self.config.block_size}"
...
def evaluate(model, validation_loader, criterion, device):
    ...
    avg_loss = total_loss / (
        len(validation_loader.dataset) * validation_loader.batch_size
    )
    return avg_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file_path", type=str, default="input.txt", help="Path to the input text file"
    )
    parser.add_argument(
        "--batch_size", type=int, default=32, help="Batch size for training"
    )
    parser.add_argument(
        "--block_size", type=int, default=128, help="Block size for training"
    )
    parser.add_argument(
        "--train_split",
        type=float,
    ...
    parser.add_argument(
        "--num_epochs", type=int, default=4, help="Number of epochs to train for"
    )
    ...
    print(
        f"Dataset split: {train_size} training samples, {validation_size} validationing samples"
    )
    ...
    print(
        f"Number of trainable parameters in the model: {sum(p.numel() for p in model.parameters() if p.requires_grad)}"
    )
    print(
        f"Size of the model {sum(p.numel() * p.element_size() for p in model.parameters()) / (1024**3):.2f}GB"
    )
    ...
    for epoch in range(num_epochs):
        ...
        for step, (x, y) in enumerate(train_loader):
            ...
            if step % 100 == 0:
                ...
                print(
                    f"Current/Maximum GPU memory used: {current_memory/(1024**3):.2f}/{max_memory/(1024**3):.2f} GB"
                )
                print(
                    f"Epoch [{epoch+1}/{num_epochs}], Step [{step}/{len(train_loader)}], Loss: {loss.item():.4f}"
                )
        ...
        print(
            f"Epoch [{epoch+1}/{num_epochs}] complete - Train Loss (per sample): {avg_train_loss:.4f}, Validation Loss (per sample): {validation_loss:.4f}"
        )
    ...
    if os.path.exists(best_model_path):
        ...
        def generate_without_cache(
            model, prompt, max_new_tokens=100, temperature=0.8, top_k=50
        ):
            model.eval()
