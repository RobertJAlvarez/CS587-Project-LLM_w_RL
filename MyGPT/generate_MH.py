...
def generate_MH(
    ...
    for _ in range(max_new_tokens):
        ...
        # --- replace from here ---
        # 3) Get temperature-scaled, top-k-filtered logits
        ...
        # 4) Build the proposal distribution q(·|context)
        probs = next_logits.softmax(dim=-1)  # shape (1, V)

        # 5) Initialize MH chain at the greedy token
        current = probs.argmax(dim=-1, keepdim=True)  # shape (1,1)
        current_logp = torch.log(probs.gather(-1, current))  # shape (1,1)

        # 6) Run m-step Metropolis–Hastings
        for _ in range(mh_steps):
            # propose a candidate from q
            prop = probs.multinomial(num_samples=1)  # (1,1)
            prop_logp = torch.log(probs.gather(-1, prop))  # (1,1)

            # acceptance probability
            alpha = torch.exp(prop_logp - current_logp)
            if torch.rand(()) < alpha:
                current, current_logp = prop, prop_logp

        next_token = current  # final sample from MH
...
if __name__ == "__main__":
    ...
    generated_text, generation_time = generate_MH(
        model=model,
        prompt=args.prompt,
        tokenizer=tokenizer,
